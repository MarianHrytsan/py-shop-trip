...
def shop_trip():
    ...
    shop_list = []
    # create car object
    for i, customer_car in enumerate(config_data["customers"]):
        car_list.append(Car(customer_car["car"]["brand"],
                            customer_car["car"]["fuel_consumption"]))
    for customer_info in config_data["customers"]:
    ...
    for customer in customer_list:
        ...
        if customer.money < cheap_shop[0]:
            ...
            print(f"{customer.name} rides to shop {shop_list[cheap_shop[1]].name} \n")
            print(f"Date: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
            print(f"Thanks, {customer.name}, for your purchase!")
            print(f"You have bought:")
            for buy_product in customer.product_cart:
                print(f"{customer.product_cart[buy_product]} {buy_product} for "
